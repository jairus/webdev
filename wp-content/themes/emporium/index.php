...
<div class="row homepage visible">
	...
	<section id="content">
		...
		$args = array( 'post_type' => 'product', 'posts_per_page' => $featured_products_per_page, 'meta_query' => array( array('key' => '_visibility','value' => array('catalog', 'visible'),'compare' => 'IN'),array('key' => '_featured','value' => 'yes')) );
		...
		while ( $loop->have_posts() ) : $loop->the_post(); global $product; ?>
				...
				<li class="<?php $i++; if( 4 == $i ) { $i = 0; echo 'last'; }?>">
					...
					<h2><a href="<?php echo get_permalink( $loop->post->ID ) ?>" title="<?php echo esc_attr($loop->post->post_title ? $loop->post->post_title : $loop->post->ID); ?>"><?php the_title(); ?> <span class="featured-price"><?php echo $product->get_price_html(); ?></span></a></h2>
		...
		<div class="col2-set">
			...
			<div class="col-1 recent-products">
				...
				<ul class="recent products">
					...
					$args = array( 'post_type' => 'product', 'posts_per_page' => $products_per_page, 'meta_query' => array( array('key' => '_visibility','value' => array('catalog', 'visible'),'compare' => 'IN'),array('key' => '_featured','value' => 'no')) );
					$loop = new WP_Query( $args );
					while ( $loop->have_posts() ) : $loop->the_post(); global $product; ?>
