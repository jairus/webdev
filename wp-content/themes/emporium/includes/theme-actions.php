...
if (!function_exists( 'woo_custom_styling')) {
	function woo_custom_styling() {
		...
		// Add CSS to output
		if ($body_color)
			$output .= 'html {background:'.$body_color.'}' . "\n";
...
if ( ! function_exists( 'woo_load_frontend_css' ) ) {
	function woo_load_frontend_css () {
		wp_register_style( 'woo-layout', get_template_directory_uri() . '/css/layout.css','','','only screen and (min-width: 768px)' );
		wp_register_style( 'woo-layout-IE', get_template_directory_uri() . '/css/layout.css' );
		
		wp_enqueue_style( 'woo-layout' );
		wp_enqueue_style( 'woo-layout-IE' );
		
		global $wp_styles;
		$wp_styles->add_data( 'woo-layout-IE', 'conditional', 'lte IE8' );
	...
	function woo_custom_typography() {
		...
		if ( $woo_options[ 'woo_typography' ] == "true") {
			...
			if ( $woo_options[ 'woo_font_nav' ] )
				$output .= '.main-navigation a, .top, .top h1 { '.woo_generate_font_css($woo_options[ 'woo_font_nav' ], '1.4').' }' . "\n";
			...
			if ( $woo_options[ 'woo_font_post_entry' ] )
				$output .= '.post, .post p { '.woo_generate_font_css($woo_options[ 'woo_font_post_entry' ], '1.5').' } h1, h2, h3, h4, h5, h6 { font-family: '.stripslashes($woo_options[ 'woo_font_post_entry' ]['face']).', arial, sans-serif; }'  . "\n";
		...
		// Add default typography Google Font
			...
			//$woo_options['woo_just_face'] = array('face' => 'Cabin');
			//$output .= 'body, h1, h2, h3, h4, h5, h6, .widget h3, .post .title, .section .post .title, .archive_header, .entry, .entry p, .post-meta, .feedback blockquote p, #post-entries, #breadcrumbs { '.woo_generate_font_css($woo_options['woo_just_face']).' }' . "\n";
...
function woo_feedburner_link ( $output, $feed = null ) {
	...
	if ( isset($woo_options[ 'woo_feed_url' ]) && ( $feed == $default ) && ( ! stristr( $output, 'comments' ) ) ) $output = esc_url( $woo_options[ 'woo_feed_url' ] );
